...
void TFatimaTwinpeaksConfiguration::ReadConfiguration()
{
    ...
    if (detector_map_file.fail()) sfsLOG(fatal, "Could not open Fatima Twinpeaks allocation file");
...
void TFatimaTwinpeaksConfiguration::ReadCalibrationCoefficients(){
    ...
    if (calibration_coeff_file.fail()) sfsLOG(fatal, "Could not open Fatima calibration coefficients file.");
...
void TFatimaTwinpeaksConfiguration::ReadTimeshiftCoefficients()
{
    ...
    if (timeshift_file.fail()) sfsLOG(fatal, "Could not open FatimaTwinpeaks timeshifts");    
    int rdetector_id; // temp read variables
    double t0;
    ...
    while(!timeshift_file.eof()){
        if(timeshift_file.peek()=='#') timeshift_file.ignore(256,'\n');
            ...
            sfsLOG(info,Form("det = %i , t = %f ns",rdetector_id,t0));
...
void TFatimaTwinpeaksConfiguration::ReadPromptFlashCut()
{
    ...
    {
        sfsLOG(warn, "FatimaTwinpeaks prompt flash cut file provided (" << promptflash_cut_file << ") is not a ROOT file.");
        return;
    }
    ...
    {
        sfsLOG(warn, "FatimaTwinpeaks prompt flash cut does not exist in file: " << promptflash_cut_file);
    }

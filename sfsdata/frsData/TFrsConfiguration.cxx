...
{   
    ...
    if (file.fail()) { sfsLOG(warn, "Could not open FRS Scaler mapping file"); return; }
...
void TFrsConfiguration::ReadTravMusDriftFile()
{
    ...
    if (travmus_drift_coeff_file.fail()) { sfsLOG(warn, "Could not open Trav Mus drift coefficients file."); return; }
    ...
    sfsLOG(info, "Trav Mus Drift File: " + tm_drift_coeff_file);
    return;    
}
...
void TFrsConfiguration::ReadAoQDriftFile()
{
    ...
    if (AoQ_drift_coeff_file.fail()) sfsLOG(warn, "Could not open AoQ drift coefficients file.");
    ...
    sfsLOG(info, "AoQ Drift File: " + aoq_drift_coeff_file);
    return;    
}
...
void TFrsConfiguration::ReadZ1DriftFile()
{
    ...
    if (Z1_drift_coeff_file.fail()) sfsLOG(warn, "Could not open Z1 drift coefficients file.");
    ...
    sfsLOG(info, "Z1 Drift File: " + z1_drift_coeff_file);
    return;    
}
...
void TFrsConfiguration::ReadCrateMapFile()
{
    ...
    if (file.fail()) sfsLOG(fatal, "Could not open FRS Crate Mapping mapping file.");

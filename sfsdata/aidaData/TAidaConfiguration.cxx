...
void TAidaConfiguration::ReadConfiguration()
{
  ...
  if (!cfg.good())
  {
    sfsLOG(fatal, "Unable to read AIDA configuration file");
    return;
  }
  ...
  while (cfg)
  {
    ...
    else if (option == "dssd")
    {
      line >> sub_DSSD;
      if (sub_DSSD > dssds) {
        sfsLOG(fatal, "DSSD value " << sub_DSSD << " exceeds number of DSSDs: " << dssds);
        sub_DSSD = 0;
      }
      else {
  ...
  {
    sfsLOG(error, "Error parsing AIDA configuration");
    return;
  }
  ...
  while (fs)
  {
    ...
    if (!warning && (fs_fee > fees || fs_channel > 64)) {
      sfsLOG(warning, "Invalid FEE/Channel number in AIDA_offsets.txt");
      warning = true;
    }
  ...
  {
    sfsLOG(error, "Error reading AIDA_offsets.txt");
  }
  else if (loaded && cfg.eof())
  {
    sfsLOG(info, "Loaded AIDA ADC offsets");
  }
  ...
  while (fs)
  {
    ...
    if (!warning && (fs_fee > fees)) {
      sfsLOG(warning, "Invalid FEE in AIDA_times.txt");
    }
  ...
  if (loaded && !cfg.eof() && (cfg.bad() || cfg.fail()))
  {
    sfsLOG(error, "Error reading AIDA_times.txt");
  }
  else if (loaded && cfg.eof())
  {
    sfsLOG(info, "Loaded AIDA FEE clock correction");
  }
  fs.close();
  ...
  if (loaded && !cfg.eof() && (cfg.bad() || cfg.fail()))
  {
    sfsLOG(error, "Error reading AIDA_gains.txt");
  }
  else if (loaded && cfg.eof())
  {
    sfsLOG(info, "Loaded AIDA DSSD gains");
  }
  ...
  if (loaded && !cfg.eof() && (cfg.bad() || cfg.fail()))
  {
    sfsLOG(error, "Error reading AIDA_strips.txt");
  }
  else if (loaded && cfg.eof())
  {
    sfsLOG(info, "Loaded AIDA DSSD strip thresholds");
  }

  if (useucesb && ignorembsts)
  {
    sfsLOG(warning, "Not ignoring MBS timestamps when ucesb is used");
    ignorembsts = false;
  }
...
void TAidaConfiguration::DSSDtoFEE()
{
  ...
  {
    if (d.DSSD == -1) {
      sfsLOG(fatal, "At least one DSSD was not mapped to any FEEs in AIDA configuration");
      continue;
    }

    if (d.Top > FEEs()) {
      sfsLOG(fatal, "DSSD top maps to invalid FEE: " << d.Top << " (Max fees: " << FEEs() << ")");
    }
    if (d.Right > FEEs()) {
      sfsLOG(fatal, "DSSD right maps to invalid FEE: " << d.Right << " (Max fees: " << FEEs() << ")");
    }
    if (d.Bottom > FEEs()) {
      sfsLOG(fatal, "DSSD bottom maps to invalid FEE: " << d.Bottom << " (Max fees: " << FEEs() << ")");
    }
    if (d.Left > FEEs()) {
      sfsLOG(fatal, "DSSD left maps to invalid FEE: " << d.Left << " (Max fees: " << FEEs() << ")");
    }
    ...
    if (d.LeftRight > FEEs()) {
      sfsLOG(fatal, "DSSD left-right maps to invalid FEE: " << d.LeftRight << " (Max fees: " << FEEs() << ")");
    }
    if (d.CentreLeft > FEEs()) {
      sfsLOG(fatal, "DSSD centre-left maps to invalid FEE: " << d.CentreLeft << " (Max fees: " << FEEs() << ")");
    }
    if (d.CentreRight > FEEs()) {
      sfsLOG(fatal, "DSSD centre-right maps to invalid FEE: " << d.CentreRight << " (Max fees: " << FEEs() << ")");
    }
    if (d.RightLeft > FEEs()) {
      sfsLOG(fatal, "DSSD right-left maps to invalid FEE: " << d.RightLeft << " (Max fees: " << FEEs() << ")");
    }

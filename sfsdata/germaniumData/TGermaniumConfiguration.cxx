...
,   num_febex_boards(0)
{
    ...
    else sfsLOG(fatal,"You must set TGermaniumConfiguration::SetConfigurationFile()!!");
...
{   
    ...
    if (detector_map_file.fail()) sfsLOG(fatal, "Could not open Germanium Detector Map");
...
void TGermaniumConfiguration::ReadCalibrationCoefficients()
{
    ...
    if (cal_map_file.fail()) sfsLOG(fatal, "Could not open Germanium calibration coefficients");    
...
void TGermaniumConfiguration::ReadTimeshiftCoefficients()
{
    ...
    if (timeshift_file.fail()) sfsLOG(fatal, "Could not open Germanium timeshifts");    
    int rdetector_id,rcrystal_id; // temp read variables
    double t0;
    ...
    while(!timeshift_file.eof()){
        if(timeshift_file.peek()=='#') timeshift_file.ignore(256,'\n');
            ...
            sfsLOG(info,Form("det = %i , cr = %i, t = %f",rdetector_id,rcrystal_id,t0));
            std::pair<int,int> detector_crystal = {rdetector_id,rcrystal_id};
...
void TGermaniumConfiguration::ReadPromptFlashCut()
{
    ...
    {
        sfsLOG(warn, "Germanium prompt flash cut file provided (" << promptflash_cut_file << ") failed to load!.");
        return;
    }
    ...
    {
        sfsLOG(warn, "Germanium prompt flash cut does not exist in file: " << promptflash_cut_file);
    }
    
        sfsLOG(info, "Germanium prompt flash cut file provided (" << promptflash_cut_file << ") loaded.");
...
void TGermaniumConfiguration::ReadPromptFlashCutMulti()
{
    ...
    sfsLOG(info, TString("Opening prompt flash mutli: ") + promptflash_cut_file_multi);
    ...
    {
        sfsLOG(warn, "Germanium prompt flash cut file provided (" << promptflash_cut_file_multi << ") failed to load!");
        return;
    }
    ...
    sfsLOG(info,"Read " << prompt_flash_cut_multi.size() << " prompt flash cuts");

    if (prompt_flash_cut_multi.size() == 0) // Log warning if no cut found at all
    {
        sfsLOG(warn, "No prompt flash cuts found in file: " << promptflash_cut_file_multi);
    }

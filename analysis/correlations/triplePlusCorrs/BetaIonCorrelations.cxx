...
InitStatus BetaIonCorrelations::Init()
{
    ...
    sfsLOG_IF(fatal, !fHitbPlastTwinpeaks, "Branch bPlastTwinpeaksCalData not found!");
...
{
    sfsLOG(info, "TERMINATION: " << fNEvents << " EVENTS PROCESSED!");


    sfsLOG(info, "Total implants: " << nImplants);
    sfsLOG(info, "Total ID correlated pairs: " << nID_Pairs);
    sfsLOG(info, "Total gammas plotted: " << nGammas);
    sfsLOG(info, "Total pass FRS: " << nPass);
    sfsLOG(info, "Gammas in Range: " << nGammasInRange);
    sfsLOG(info, "Gammas out of Range: " << nGammasOutOfRange);
    ...
    if (found_dir_corrs == false) // && FairRunOnline::Instance() == NULL 
    {
        ...
        dir_corrs->Write();
        gDirectory = tmp;
        sfsLOG(info, "Written Beta-Ion Correlations histograms to file.");
    }
}

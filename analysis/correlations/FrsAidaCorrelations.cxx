...
{
    sfsLOG(info, "Deleting FrsAidaCorrelations task.");
}
...
InitStatus FrsAidaCorrelations::Init()
{
    ...
    sfsLOG_IF(fatal, !header, "Branch EventHeader. not found!");
...
void FrsAidaCorrelations::FinishTask()
{
    if (found_dir_frs == false && FairRunOnline::Instance() == NULL)
    {
        ...
        dir_frs->Write();
        gDirectory = tmp;
        sfsLOG(info, "Written FRS-AIDA Correlations histograms to file.");
    }
}

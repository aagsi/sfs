	...
	for (int j = 0; j < xmax; j++)
	  {
		...
		sfsLOG(info, "layer: " << i << " x max: " << xmax << " ymax: " << ymax << " j: " << j << " k: " << k<< " city: " << city<< " canvas " << i*(xmax*ymax)+ymax*j+k);
...
void LisaTraceAnalysis::Exec(Option_t* option){
  ...
  for (auto const & lisaCalItem : *lisaCalArray){
    ...
    //sfsLOG(info,"layer uncorrected " << layer);
    // remove tokyo
    if(layer<1)
      continue;
    layer-=1;
    ...
    //sfsLOG(info,"hit in x = " << xpos << ", y = " << ypos << ", z = " << layer << ", e = " <<energy);
        
    // //::::::::F I L L   H I S T O S:::::::
 
    //sfsLOG(info,"filling h2_traces_hit[layer*(xmax*ymax)+ymax*xpos+ypos] " << layer*(xmax*ymax)+ymax*xpos+ypos);
  ...
  for (auto const & lisaCalItem : *lisaCalArray){
    ...
    if(xpos == xhit && ypos == yhit)
      continue;
    //sfsLOG(info,"not hit in x = " << xpos << ", y = " << ypos << ", z = " << layer);
    //sfsLOG(info,"filling h2_traces_nothit[layer*(xmax*ymax)+ymax*xpos+ypos][ymax*xpos+ypos] " << layer*(xmax*ymax)+ymax*xpos+ypos << " " << ymax*xpos+ypos);
    std::vector<uint16_t> trace = lisaCalItem.Get_trace();
    for (int t = 0; t < trace.size(); t++)
  ...
  //sfsLOG(info,"done with event " << fNEvents);
  fNEvents += 1;
}

...
Bool_t FatimaTimingAnalysis::SetDetectorTimeshifts(TString filename){
    sfsLOG(info, "Reading Timeshift coefficients.");
    sfsLOG(info, "File reading");
    sfsLOG(info, filename);
...
InitStatus FatimaTimingAnalysis::Init()
{
    ...
    if (detector_id_analyze == nullptr) {sfsLOG(fatal, "Please set which detector ids to analyze");}
...
{   
    if (fHitFatimaTwinpeaks && fHitFatimaTwinpeaks->GetEntriesFast() > 0)
    {
        ...
        {   
            ...
            //sfsLOG(info,energy1);
            //sfsLOG(info,SoftwareTimewalk(energy1));

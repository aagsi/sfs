...
{
    sfsLOG(info, "Destroyed AidaPulserWalkthrough properly.");
...
InitStatus AidaPulserWalkthrough::Init()
{
    ...
    sfsLOG_IF(fatal, !adcArray, "Branch AidaAdcData not found!");
    flowArray = mgr->InitObjectAs<decltype(flowArray)>("AidaFlowData");
    sfsLOG_IF(fatal, !adcArray, "Branch AidaFlowData not found!");
    scalerArray = mgr->InitObjectAs<decltype(scalerArray)>("AidaScalerData");
    sfsLOG_IF(fatal, !adcArray, "Branch AidaScalerData not found!");

    // Calibrated data
    implantCalArray = mgr->InitObjectAs<decltype(implantCalArray)>("AidaImplantCalAdcData");
    sfsLOG_IF(fatal, !implantCalArray, "Branch AidaImplantCalAdcData not found!");
    decayCalArray = mgr->InitObjectAs<decltype(decayCalArray)>("AidaDecayCalAdcData");
    sfsLOG_IF(fatal, !decayCalArray, "Branch AidaDecayCalAdcData not found!");

    // Hit data
    implantHitArray = mgr->InitObjectAs<decltype(implantHitArray)>("AidaImplantHits");
    sfsLOG_IF(fatal, !implantHitArray, "Branch AidaImplantHits not found!");
    decayHitArray = mgr->InitObjectAs<decltype(decayHitArray)>("AidaDecayHits");
    sfsLOG_IF(fatal, !decayHitArray, "Branch AidaDecayHits not found!");
...
void AidaPulserWalkthrough::FinishTask()
{
    if (fNEvents == 0)
    {
        sfsLOG(warn, "No events found, not saving histograms!");
        return;
    }
    if (fNEvents > 0)
    {
        sfsLOG(info, "Average execution time: " << (double)total_time_microsecs/fNEvents << " microseconds.");
    }
    histos->Write();
}
...
void AidaPulserWalkthrough::FindOffsets()
{
    ...
    sfsLOG(info, "Beginning pulser offset calibation");
    ...
    if (!calf || !logf)
    {
        sfsLOG(error, "Could not open output files :(");
        return;
    }
    ...
    for (int FEE = 1; FEE <= conf->FEEs(); FEE++)
    {
        ...
        sfsLOG(info, "For FEE#" << FEE << " " << good << "/64 channels calibrated without comment");
    }
    sfsLOG(info, "Pulser calibration complete, please check the log for channels to manually fix");

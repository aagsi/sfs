...
InitStatus BGOOnlineSpectra::Init()
{
    ...
    sfsLOG_IF(fatal, !fHitBGO, "Branch BGOTwinpeaksData not found!");
    ...
    for (auto it_mapping = bgomap.begin(); it_mapping != bgomap.end(); ++it_mapping)
    {
        ...
        if (std::find_if(gmap.begin(), gmap.end(), [&](const auto& m2_pair) {
            ...
            sfsLOG(warning, Form("The following bgo crystal %d%c was not found in the germanium detector map.",detector_crystal_pair.first,(char)(detector_crystal_pair.second+65)));
        }
    }

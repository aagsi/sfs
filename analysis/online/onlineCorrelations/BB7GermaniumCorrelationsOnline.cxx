...
BB7GermaniumCorrelationsOnline::~BB7GermaniumCorrelationsOnline()
{
    sfsLOG(info, "Deleting BB7GermaniumCorrelationsOnline task.");
}
...
InitStatus BB7GermaniumCorrelationsOnline::Init()
{
    ...
    sfsLOG_IF(fatal, !bb7empty, "Branch BB7EmptyData not found!");
    bb7rawHits = mgr->InitObjectAs<decltype(bb7rawHits)>("BB7FebexData");
    sfsLOG_IF(fatal, !bb7rawHits, "Branch BB7FebexData not found!");
    ...
    sfsLOG_IF(fatal, !bb7calImplants, "Branch BB7FebexCalImplants not found!");
    sfsLOG_IF(fatal, !bb7calDecays, "Branch BB7FebexCalDecays not found!");

    bb7decays = mgr->InitObjectAs<decltype(bb7decays)>("BB7FebexDecays");
    sfsLOG_IF(fatal, !bb7decays, "Branch BB7FebexDecays not found!");
    bb7implants = mgr->InitObjectAs<decltype(bb7implants)>("BB7FebexImplants");
    sfsLOG_IF(fatal, !bb7implants, "Branch BB7FebexImplants not found!");

    germaniumHits = (TClonesArray*)mgr->GetObject("GermaniumCalData");
    sfsLOG_IF(fatal, !germaniumHits, "Branch GermaniumCalData not found!");
    ...
    {   
        sfsLOG(info, "Creating DEGAS Directory");
...
{
    sfsLOG(info, Form("BB7 WR w/ F0:: %i", n_bb7_f0));
    sfsLOG(info, Form("BB7 WR empty:: %i", n_bb7_empty));
    sfsLOG(info, Form("BB7 WR w/ F0 Coinc w/ Germanium:: %i", n_coinc_f0));
    sfsLOG(info, Form("BB7 WR empty Coinc w/ Germanium:: %i", n_coinc_empty));

    sfsLOG(info, Form("Cal Decay Events:: %i", n_cal_decay));
    sfsLOG(info, Form("Cal Implant Events:: %i", n_cal_implant));
    sfsLOG(info, Form("Cal Decay Coinc w/ Germanium:: %i", n_coinc_cal_decay));
    sfsLOG(info, Form("Cal Implant Coinc w/ Germanium:: %i", n_coinc_cal_imp));

    sfsLOG(info, Form("Clustered Decays Events:: %i", n_decay_event));
    sfsLOG(info, Form("Clustered Implant Events:: %i", n_implant_event));
    sfsLOG(info, Form("Clustered Decay Coinc w/ Germanium :: %i", n_coinc_decay));
    sfsLOG(info, Form("Clustered Implant w/ Germanium:: %i", n_coinc_imp));

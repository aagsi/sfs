...
InitStatus WhiterabbitCorrelationOnline::Init()
{
    ...
    sfsLOG_IF(error, !fEventHeader, "Branch EventHeader. not found");
    ...
    for (int i = 0; i < fNumDetectorSystems; i++)
    {
        ...
        else if (fDetectorSystems.at(i) == "BB7") // make adaptable to spellings...
        {
            ...
            sfsLOG_IF(fatal, !fBB7Decays, "Branch BB7DecayData not found!");
            fBB7Implants = mgr->InitObjectAs<decltype(fBB7Implants)>("BB7ImplantData");
            sfsLOG_IF(fatal, !fBB7Implants, "Branch BB7ImplantData not found!");  
        }
        else
        {
            sfsLOG(fatal, "Unknown detector system: " << fDetectorSystems.at(i));
        }
    }
...
void WhiterabbitCorrelationOnline::Reset_Histo() {
    sfsLOG(info, "Resetting White Wabbit histograms.");

    // Assuming dir is a TDirectory pointer containing histograms
    if (dir_whiterabbit) {
        AnalysisTools_H::ResetHistogramsInDirectory(dir_whiterabbit);
        sfsLOG(info, "White Wabbit histograms reset.");
    } else {
        sfsLOG(error, "Failed to get list of histograms from directory.");
    }
}
...
void WhiterabbitCorrelationOnline::FinishTask()
{
    if (fNEvents == 0)
    {
        sfsLOG(warning, "No events found, no histograms written.");
        return;
    }
   
    sfsLOG(info, "Processed " << fNEvents << " events.");

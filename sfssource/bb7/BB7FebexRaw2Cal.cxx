...
InitStatus BB7FebexRaw2Cal::Init()
{
    ...
    sfsLOG_IF(fatal, !bb7array, "Branch BB7FebexData not found!");
...
void BB7FebexRaw2Cal::Exec(Option_t* option)
{
    ...
    for (auto const & bb7item : *bb7array)
    {
        ...
        if (bb7_config->MappingLoaded())
        {
            ...
            // else sfsLOG(warn, "Unmapped data? Board: "  << unmapped_channel.second.first << " Channel: " << unmapped_channel.second.second);
...
{
    sfsLOG(info, Form("Unmapped item:: %i", nUnmapped));
    sfsLOG(info, Form("Nothing items:: %i", nNothings));
}
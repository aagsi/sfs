...
{
    sfsLOG(info, "Counter: " << counter);
    sfsLOG(info, "unmatched lead trail counter: " << lead_trail_unmatched_counter);
    sfsLOG(info, "fNEvents: " << fNEvent);
}
...
void FimpReader::WriteFineTimeHistosToFile()
{
    ...
    {
        sfsLOG(warn, "Fine time calibrations not set, cannot write to file!");
        return;
    }

    TFile* outputfile = TFile::Open(Form("%s", fine_time_histo_outfile.Data()), "RECREATE");

    if (!outputfile->IsOpen()) { sfsLOG(warn, "Fine Time histogram file was not opened, cannot write calibrations to file!"); return; }
    ...
    sfsLOG(info, "You have successfully done fine time calibration. These are written to file - please restart the program and add set the correct fine time calibration file for FimpReader.");
}

void FimpReader::ReadFineTimeHistosFromFile()
{
    TFile* inputfile = TFile::Open(fine_time_histo_infile, "READ");
    if (!inputfile || inputfile->IsZombie()) sfsLOG(fatal, "Fine time calibration file could not be opened, aborting program.");
    ...
    for (int i = 0; i < chans_per_tdc; i++)
    {
        ...
        if (a)
        {
            ...
            sfsLOG_IF(fatal, fine_time_hits == nullptr, "Failed reading fine time calibration histograms - aborting!");
            delete a;
            a = nullptr;
    ...
    for (uint32_t channel_index = 0; channel_index < fData->fimp_data_time_coarseM; channel_index++)
    {
        ...
        for (uint32_t j = hit_index; j < next_channel_start; j++)
        {
            ...
            if (channel != prev_channel && channel < 128)
            {
                ...
                {
                    sfsLOG(debug2, "Unmatched trail(s). Channel = " << channel << " Prev channel = " << prev_channel);
                }
            ...
            else if (channel == prev_channel && channel < 128)
            {
                ...
                {
                    sfsLOG(warn, "Unmatched leads or trails/leads swapped? Should we account for this?");
                }
            ...
            else if (channel < 128)
            {
                ...
                {   
                    ...
                    {
                        unmatchedLeads++;
                        //sfsLOG(warn, "Unmatched lead in FIMP data: " << unmatchedLeads);
                    }
                ...
                {   
                    if (last_hit.leadOrTrail == -1) //sfsLOG(warn, "First data item is a trail");
                    {}
                    else if (last_hit.leadOrTrail == 1) 
                    {
                        //sfsLOG(warn, "Two trails in a row");
                        continue;
                    }
                    else
                    {   
                        ...
                        {
                            sfsLOG(warn, "Couldn't match lead and trail channels!");

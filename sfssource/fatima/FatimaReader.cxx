...
FatimaReader::~FatimaReader() { 
    ...
    sfsLOG(info, "Destroyed FatimaReader properly.");
...
void FatimaReader::WriteFineTimeHistosToFile()
{
    ...
    sfsLOG(info,"You have successfully done fine time calibration. These are written to file. Please restart the program and add ReadFineTimehistosFromFile instead for FatimaReader.\n (yeah this is not a fatal error per se just restart it with your fresh calibrations :) )");
...
Bool_t FatimaReader::Read() //do fine time here:
{
    ...
    { //per board:
        ...
        //sfsLOG(info,Form("event size: %i",fData->fatima_tamex[it_board_number].event_size));
        ...
        for (int it_hits = 0; it_hits < fData->fatima_tamex[it_board_number].event_size/4 - 3 ; it_hits++)
        {
            ...
            */
            ...
            if (fData->fatima_tamex[it_board_number].time_epochv[it_hits] != 0)
            {
                    ...
                    //if (it_board_number == 1) sfsLOG(info,Form("Found epoch for ch = %i, e = %i",next_channel,fData->fatima_tamex[it_board_number].time_epochv[it_hits] & 0xFFFFFFF));
                    fNepochwordsread++;
            ...
            //if (it_board_number == 1) sfsLOG(info,Form("ch = %i, coarse = %i, edge = %i", channelid, fData->fatima_tamex[it_board_number].time_coarsev[it_hits], fData->fatima_tamex[it_board_number].time_edgev[it_hits]));
            ...
            if (channelid != 0 && channelid != last_channel_read && !last_word_read_was_epoch){fNevents_lacking_epoch[it_board_number][channelid]++; sfsLOG(debug2, "Event lacking epoch.");} // if the channel has changed but no epoch word was seen in between, channel 0 is always the first one so dont check if that s the case.
            ...
            //if(it_board_number == 1) sfsLOG(info,fData->fatima_tamex[it_board_number].time_edgev[it_hits]);
            ...
            { 
                ...
                //if (it_board_number == 1) sfsLOG(info,Form("Writing: ch = %i, le = %i lc = %i, lf = %f, te = %i tc = %i, tf = %f ",channelid,last_tdc_hit.lead_epoch_counter, last_tdc_hit.lead_coarse_T, last_tdc_hit.lead_fine_T,last_tdc_hit.lead_epoch_counter,coarse_T,fine_T));

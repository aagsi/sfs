...
*/
FatimaRaw2Cal::~FatimaRaw2Cal(){
    sfsLOG(info, "Deleting FatimaRaw2Cal task");
...
{  
    ...
    sfsLOG_IF(fatal, !funcal_data, "Fatima branch of FatimaTwinpeaksData not found.");
        ...
        for (Int_t ihit = 0; ihit < event_multiplicity; ihit++)
        {
            ...
            if (fatima_configuration->MappingLoaded())
            {
                ...
                if (auto result_find = fmap.find(unmapped_det); result_find != fmap.end())
                {
                    ...
                    //sfsLOG(info,Form("board = %i, ch = %i, det = %i",funcal_hit->Get_board_id(), (funcal_hit->Get_ch_ID()+1)/2,result_find->second));
            ...
            //sfsLOG(info,Form("board = %i, ch = %i, det = %i",funcal_hit->Get_board_id(),(int)((funcal_hit->Get_ch_ID()+1)/2), detector_id));

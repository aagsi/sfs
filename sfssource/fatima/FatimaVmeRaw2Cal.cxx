...
FatimaVmeRaw2Cal::~FatimaVmeRaw2Cal()
{
    sfsLOG(info, "Deleting FatimaVmeRaw2Cal task");
...
InitStatus FatimaVmeRaw2Cal::Init()
{
    ...
    sfsLOG_IF(fatal, !funcal_data, "Fatima branch of FatimaVmeData not found!");
    ...
    sfsLOG_IF(fatal, !qdcArray, "Branch qdcArray not found!");

    tdcArray = mgr->InitObjectAs<decltype(tdcArray)>("FatimaVmeTDCData");
    sfsLOG_IF(fatal, !tdcArray, "Branch tdcArray not found!");
        ...
        for (Int_t ihit = 0; ihit < nHits; ihit++)
        {
            ...
            for (int i = 0; i < dummyqdcmult; i++)
            {
                if (dummy_qdc_id[i] == dummy_tdc_id[i] && EnID[i] && TimID[i] == 0)
                {
                    ...
                    if (dummy_qdc_id[i] == -1)
                    {
                        sfsLOG(warn, "There was a -1 match in Fatima VME");
                        matchedmult--;
                    }
                }
                else

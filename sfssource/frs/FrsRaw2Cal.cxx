...
FrsRaw2Cal::~FrsRaw2Cal()
{
    sfsLOG(info, "Deleting FrsRaw2Cal task");
}
...
InitStatus FrsRaw2Cal::Init()
{
    ...
    sfsLOG_IF(fatal, !sciArray, "Branch FrsSciData not found!");

    tpcArray = mgr->InitObjectAs<decltype(tpcArray)>("FrsTpcData");
    sfsLOG_IF(fatal, !tpcArray, "Branch FrsTpcData not found!");

    musicArray = mgr->InitObjectAs<decltype(musicArray)>("FrsMusicData");
    sfsLOG_IF(fatal, !musicArray, "Branch FrsMusicData not found!");

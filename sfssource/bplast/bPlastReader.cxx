...
bPlastReader::~bPlastReader() { 
    ...
    sfsLOG(info, "Average execution time: " << (double)total_time_microsecs/fNEvent << " microseconds.");
    sfsLOG(info, "Events: " << fNEvent);
    sfsLOG(info, "Destroyed bPlastReader properly.");
...
void bPlastReader::DoFineTimeCalibration()
{
    ...
    for (int i = 0; i < NBoards; i++) {
        for (int j = 0; j < NChannels; j++) {
            ...
            if (total_counts == 0) 
            {
                ...
                warning_counter++;
                sfsLOG(debug2, Form("Channel %i on board %i does not have any fine time hits in the interval.",j,i));
    ...
    if (warning_counter > 0) sfsLOG(warning, Form("%i channels do not have any fine time hits in the interval.", warning_counter));
    fine_time_calibration_set = true;
...
void bPlastReader::ReadFineTimeHistosFromFile() 
{
    ...
    for (int i = 0; i < NBoards; i++) {
        ...
        for (int j = 0; j < NChannels; j++) {
            ...
            if (a) {
                sfsLOG(debug2, Form("Accessing i = %i, j = %i",i,j));
        ...
        for (int it_hits = 0; it_hits < fData->bplast_tamex[it_board_number].event_size/4 - 3 ; it_hits++)
        {
            ...
            */
            ...
            if (fData->bplast_tamex[it_board_number].time_epochv[it_hits] != 0)
            {
                ...
                //if (it_board_number == 1) sfsLOG(info,Form("Found epoch for ch = %i, e = %i",next_channel,fData->bplast_tamex[it_board_number].time_epochv[it_hits] & 0xFFFFFFF));
                fNepochwordsread++;
            ...
            uint32_t channelid = fData->bplast_tamex[it_board_number].time_channelv[it_hits] & 0x7F; // 0-32
            
            //if (it_board_number == 1) sfsLOG(info,Form("ch = %i, coarse = %i, edge = %i", channelid, fData->bplast_tamex[it_board_number].time_coarsev[it_hits], fData->bplast_tamex[it_board_number].time_edgev[it_hits]));
            
            if (fData->bplast_tamex[it_board_number].time_finev[it_hits] == 0x3FF) {fNevents_TAMEX_fail[it_board_number][channelid]++; continue;} // this happens if TAMEX loses the fine time - skip it
            
            // why channelid-1 and why skip 0 above. fatima does not do this?
            if (channelid != 0 && channelid != last_channel_read && !last_word_read_was_epoch){fNevents_lacking_epoch[it_board_number][channelid]++; sfsLOG(debug2, Form("Event lacking epoch in channel %d",channelid)); continue;} // if the channel has changed but no epoch word was seen in between, channel 0 is always the first one so dont check if that s the case.
            ...
            {
                fine_time_hits[it_board_number][channelid]->Fill(fData->bplast_tamex[it_board_number].time_finev[it_hits]);
                continue;
            }

            uint32_t coarse_T = fData->bplast_tamex[it_board_number].time_coarsev[it_hits] & 0x7FF;
            double fine_T = GetFineTime(fData->bplast_tamex[it_board_number].time_finev[it_hits],it_board_number,channelid);
            ...
            //if(it_board_number == 1) sfsLOG(info,fData->bplast_tamex[it_board_number].time_edgev[it_hits]);
            ...
            { 
                ...
                //if (it_board_number == 1) sfsLOG(info,Form("Writing: ch = %i, le = %i lc = %i, lf = %f, te = %i tc = %i, tf = %f ",channelid,last_hits[it_board_number][channelid-1].lead_epoch_counter, last_hits[it_board_number][channelid-1].lead_coarse_T, last_hits[it_board_number][channelid-1].lead_fine_T,last_epoch[channelid-1],coarse_T,fine_T));

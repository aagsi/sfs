...
*/
bPlastRaw2Cal::~bPlastRaw2Cal(){
    sfsLOG(info, "Deleting bPlastRaw2Cal task");
...
InitStatus bPlastRaw2Cal::Init()
{
    ...
    sfsLOG_IF(fatal, !funcal_data, "bPlast branch of bPlastTwinpeaksData not found.");
...
Bool_t bPlastRaw2Cal::SetDetectorMapFile(TString filename){
    sfsLOG(info, "Reading Detector map");

    std::ifstream detector_map_file (filename);


    if (!detector_map_file.is_open()) {
        sfsLOG(fatal, Form("File '%s' does not exist", filename.Data()));
    }
    ...
    while (!detector_map_file.eof()) {
        if (detector_map_file.peek() == '#') {
            ...
            //sfsLOG(info, Form("Reading %i, %i, %i, %s, %s", rtamex_module, rtamex_channel, rdetector_id, rup_down, rleft_right_bottom_top));
...
Bool_t bPlastRaw2Cal::SetDetectorCalFile(TString filename){
    sfsLOG(info, "Reading Calibration coefficients.");
    ...
    while(!cal_map_file.eof()){
        if(cal_map_file.peek()=='#') cal_map_file.ignore(256,'\n');
            ...
            if (it != calibration_coeffs.end()) sfsLOG(fatal,Form("Calibration coefficients not unique. Multiple entries of (detector id = %i)",rdetector_id));
        }
    }
...
void bPlastRaw2Cal::PrintDetectorMap()
{
    if (DetectorMap_loaded){
        ...
        sfsLOG(info, "Detector map is not load. Cannot print.");
    }
}
...
void bPlastRaw2Cal::PrintDetectorCal(){
    if (DetectorCal_loaded){
        ...
        sfsLOG(info, "Cal map is not load. Cannot print.");
    }
}        
    ...
    if (funcal_data && funcal_data->GetEntriesFast() > 1){ // only get events with two hits.or more
        ...
        for (Int_t ihit = 0; ihit < event_multiplicity; ihit++){
            ...
            while (!all_hits_in_fast_slow_found)
            {
                ...
                //sfsLOG(info,this_hit->Get_ch_ID());
            ...
            //sfsLOG(info,Form("fast hits = %i, slow hits = %i, look ahead counter = %i",hits_in_fast_channel,hits_in_slow_channel,look_ahead_counter));
            ...
            for (int hitnr = 0; hitnr<hits_in_fast_channel; hitnr++)
            {
                ...
                if (bplast_config->MappingLoaded())
                {
                    ...
                    // else sfsLOG(warn, "Detector mapping is not complete! CEJ: Warning only for now...");
                }
                ...
                if (DetectorMap_loaded)
                {
                    ...
                    if (detector_id == -1) {fNunmatched++; continue;} //if only one event is left
                        ...
                        sfsLOG(warn, "Detector mapping not complete! CEJ: Warning only for now.");
                        //return;
                    }
                }
                else
            ...
            //if (detector_id == 0 || detector_id == 1) sfsLOG(info,Form("id = %i, fast lead = %f, fast trail = %f, fast ToT = %f",detector_id,fast_lead_time,fast_trail_time,fast_ToT));
...
{
    sfsLOG(info, "We see " << trig3 << " trigger 3 events");
    int64_t dt = (current_wr_t - first_wr_t) / 1e9;
    sfsLOG(info, "Elapsed real time is: " << dt << " seconds");
    sfsLOG(info, Form("Wrote %i events.",fNEvents));
    sfsLOG(info, Form("%i events are unmatched (not written).",fNunmatched));

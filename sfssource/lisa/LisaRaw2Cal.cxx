...
void LisaRaw2Cal::PrintDetectorGainM()
{
    ...
    {
        sfsLOG(info, "Gain matching parameters are not loaded");
    }
}   
//:::
    ...
    {          
        ...
        if (lisa_config->MappingLoaded())
        {
            if (detector_mapping.count(unmapped_channel) > 0)
            {
                ...
                //sfsLOG(info,"event num: "<< EVTno);

            }
            else sfsLOG(warn, "Unmapped data? Board: "  << unmapped_channel.first << " Channel: " << unmapped_channel.second);
